...
part of 'recently_cubits.dart';

class RecentlyCubitState {
  MediaPlaylist mediaPlaylist;
  RecentlyCubitState({required this.mediaPlaylist});

  RecentlyCubitState copyWith({MediaPlaylist? mediaPlaylist}) {
...
class ChartState {
  ChartModel chart;
  String coverImg;
  ChartState({required this.chart, required this.coverImg});

  ChartState copyWith({ChartModel? chart, String? coverImg}) {
    return ChartState(
...
class ChartInitial extends ChartState {
  ChartInitial()
    : super(chart: ChartModel(chartName: "", chartItems: []), coverImg: "");
}

class FetchChartState {
  bool isFetched;
  FetchChartState({required this.isFetched});
...
class YTMusicCubitState extends Equatable {
  ...
  const YTMusicCubitState({required this.ytmData});

  YTMusicCubitState copyWith({Map<String, List<dynamic>>? ytmData}) {
    return YTMusicCubitState(ytmData: ytmData ?? this.ytmData);
  }

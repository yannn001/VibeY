...
class DownloaderCubit extends Cubit<DownloaderState> {
  ...
  Future<String> getDownPath() async {
    return (await DBService.getSettingStr(
      GlobalStrConsts.downPathSetting,
      defaultValue: (await getDownloadsDirectory())!.path,
    ))!;
  }
  ...
  Future<void> initDownloader() async {
    ...
    receivePort.listen((dynamic data) async {
      ...
      DownTask? _task;
      try {
        _task = downloadedSongs.firstWhere(
        ...
        if (status == DownloadTaskStatus.complete.index) {
          ...
          if (_task.song.extras!['source'] != 'youtube') {
            ...
            if (file.existsSync()) {
              ...
              log(
                "Renamed ${_task.fileName} to ${_task.fileName.replaceAll(".mp4", ".m4a")}",
                name: "DownloaderCubit",
              );
            }
          }

          DBService.putDownloadDB(
          ...
          log(
            "Failed to download ${_task.song.title}",
            name: "DownloaderCubit",
          );
        } else {}
      }

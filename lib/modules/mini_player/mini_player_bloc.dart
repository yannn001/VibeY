...
class MiniPlayerBloc extends Bloc<MiniPlayerEvent, MiniPlayerState> {
  ...
  VibeyPlayerCubit playerCubit;
  Stream? combinedStream;
  MiniPlayerBloc({required this.playerCubit}) : super(MiniPlayerInitial()) {
    combinedStream = Rx.combineLatest2(
      playerCubit.vibeyplayer.audioPlayer.playerStateStream,
      playerCubit.vibeyplayer.mediaItem,
  ...
  void buffering(
    MiniPlayerBufferingEvent event,
  ...
  void processing(
    MiniPlayerProcessingEvent event,
    Emitter<MiniPlayerState> emit,
  ) {
  ...
  void completed(
    MiniPlayerCompletedEvent event,
    Emitter<MiniPlayerState> emit,
  ) {
  ...
  void listenToPlayer() {
    _playerStateSubscription = combinedStream?.asBroadcastStream().listen((
      event,
    ) {

...
import 'package:vibey/Repo/JioMusic/format.dart';
...
class SaavnAPI {
  ...
  Future<Map> getRelated(String id) async {
    ...
    if (response.statusCode == 200) {
      ...
      return {'songs': List.empty(), 'error': response.body, 'total': 0};
    }
  }
  ...
  Future<List<Map>> fetchSearchResults(String searchQuery) async {
    ...
    try {
      if (res.statusCode == 200) {
        ...
        searchedAlbumList = await formatAlbumResponse(
          albumResponseList,
          'album',
        );
  ...
  Future<Map<String, dynamic>> fetchAlbumDetails(String token) async {
    ...
    if (response.statusCode == 200) {
      ...
      final List responseList = getMain['list'] as List;
      final Map albumDetails = await formatSearchedAlbumResponse(getMain);
      final List songs = await formatSongsResponse(responseList, 'song');
      return {'albumDetails': albumDetails, 'songs': songs};
    } else {
      log("Request failed with status: ${response.statusCode}");
      return {
  ...
  Future<Map<String, dynamic>> fetchArtistDetails(
    ...
    if (response.statusCode == 200) {
      ...
      final List albums = await formatAlbumResponse(
        albumResponseList,
        'albumSearched',
      );
      return {'artistDetails': artistDetails, 'songs': songs, 'albums': albums};
    } else {
      log("Request failed with status: ${response.statusCode}");
      return {
  ...
  Future<Map<String, dynamic>> fetchPlaylistDetails(
    ...
    if (response.statusCode == 200) {
      ...
      // log(getMain.toString());
      final List responseList = getMain['list'] as List;
      final Map playlistDetails = await formatSinglePlaylistResponse(getMain);
      final List songs = await formatSongsResponse(responseList, 'song');
      return {'playlistDetails': playlistDetails, 'songs': songs};
    } else {
      log("Request failed with status: ${response.statusCode}");
      return {

...
Future<Map> formatSingleSongResponse(Map response) async {
  ...
  try {
    ...
    return {
      ...
      'type': response['type'],
      'album':
          response['album']?.toString().unescape() ??
      ...
      'title':
          response['song']?.toString().unescape() ??
      ...
      'url':
          (response['encrypted_media_url']?.toString() ??
                      response['more_info']?['encrypted_media_url']
                          ?.toString()) !=
                  null
              ? decode(
                (response['encrypted_media_url']?.toString() ??
...
Future<Map> formatSearchedAlbumResponse(Map response) async {
  try {
    ...
    return {
      ...
      'token':
          Uri.parse(
            response['perma_url'] ?? response['url'].toString(),
          ).pathSegments.last,
      'count':
...
Future<Map> formatSinglePlaylistResponse(Map response) async {
  try {
    return {
      ...
      'artist':
          (response['artist_name']?.join(', ')?.toString().unescape() ??
      ...
      'image': getImageUrl(response['image'].toString()),
      'perma_url':
          response['perma_url']?.toString().unescape() ??

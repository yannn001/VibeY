...
class YouTubeServices {
  ...
  factory YouTubeServices({String? appDocPath, String? appSuppPath}) {
    if (appDocPath != null) {
      _instance.docPath = appDocPath;
    }
    if (appSuppPath != null) {
      _instance.supPath = appSuppPath;
    }
  ...
  Future<Video?> getVideoFromId(String id) async {
    try {
      ...
      log(
        'Error while getting video from id ${e.toString()}',
        name: "YoutubeAPI",
      );
      return null;
    }
  ...
  Future<Map?> formatVideoFromId({
    ...
    final Map? response = await formatVideo(
      video: vid,
      quality:
          await DBService.getSettingStr('ytQuality', defaultValue: 'Low') ??
          'Low',
      data: data,
      getUrl: getUrl ?? true,
      // preferM4a: Hive.box(
  ...
  Future<Map?> formatVideo({
    ...
    if (getUrl) {
      // check cache first
      if (checkCache) {
        final ytCache = await DBService.getYtLinkCache(video.id.value);
        if (ytCache != null) {
  ...
  Future<List<Map>> fetchSearchResults(
    String query, {
    bool playlist = false,
  }) async {
    if (playlist) {
      final List<SearchResult> searchResults = await yt.search.searchContent(
        query,

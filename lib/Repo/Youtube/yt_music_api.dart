...
import 'package:vibey/utils/extentions.dart';
...
class YtMusicService {
  ...
  initLanguage() async {
    if (context == null) {
      throw Exception('Context is null in initLanguage');
    }
          ...
          for (final element in subtitleList) {
            ...
            if (browseEndpoint['type'] == 'MUSIC_PAGE_TYPE_ARTIST') {
              ...
              if (element["text"] != Null &&
                ...
                details['artists'].add({
                  'name': (element['text'] ?? "").toString().replaceAll(
                    ', & ,',
                    '&',
                  ),
  ...
  Future<Map> getSongData({required String videoId}) async {
    ...
    try {
      ...
      final Map response2 = await sendRequest(
        endpoints['next']!,
        body,
        headers,
      );
    ...
    try {
      ...
      if (matches.isNotEmpty && matches.toList()[1].group(1) != null) {
        ...
        Map<String, dynamic> data = json.decode(
          decodedString.unescape().replaceAll('\\"', '"'),
        );
        response = data;
      } else {
      ...
      if (isRemaining) {
        ...
        while (isRemaining) {
          ...
          Map response2 = await sendRequest(
            endpoints['browse']!,
            body,
            headers,
          );
      ...
      final String? heading = nav(tab, ['title', 'runs', 0, 'text']) as String?;
      final String subtitle =
          (nav(tab, ['subtitle', 'runs']) as List? ?? [])
              .map((e) => e['text'])
              .toList()
              .join();
      final String? description =
      ...
      await Future.forEach(finalResults, (item) async {
        ...
        String image;
        if ((nav(item, ['musicResponsiveListItemRenderer']) as Map).containsKey(
          'thumbnail',
        )) {
          image =
              nav(item, [
        ...
        if ((nav(item, [
          ...
          final sub = (nav(tab, ['straplineTextOne', 'runs']) as List);
          for (var i in sub) {
            artist += i['text'];
          }
        }
    ...
    try {
      ...
      if (nextTabTitle.contains('Albums')) {
        ...
        for (final e in nextTab) {
          final String id =
              nav(e, [mTRIR, ...navigationBrowseId]) as String? ?? '';
          final String image =
              nav(e, [mTRIR, ...thumbnailRenderer, 0, 'url']) as String? ?? '';
          final String ttl = nav(e, [mTRIR, ...titleText]) as String? ?? '';
          final List subtitle = nav(e, [mTRIR, ...subtitleRuns]) as List? ?? [];
  ...
  Future<Map<String, dynamic>> getRelated(String id) async {
    ...
    try {
      ...
      return {'songs': results, 'total': results.length, 'id': playlistId};
    } catch (e) {
      dev.log('Error in ytmusic getRelated', name: "YTM", error: e);
      return {'songs': [], 'total': 0, 'id': ''};
    }
  }
}

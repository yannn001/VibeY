...
Future<List<ChartModel>> fetchTrendingVideos() async {
  ...
  var response = await http.get(
    Uri.parse('https://charts.youtube.com/charts/TrendingVideos/gb'),
  );
  ...
  if (response.statusCode == 200) {
    ...
    // }
    DBService.putChart(chartModels[0]);
    log(
      "Trending Charts: ${chartModels[0].chartItems?.length} items loaded",
      name: "Trending YT Charts",
    );
    return chartModels;
  } else {
    final chart = await DBService.getChart("Trending Videos");
    if (chart != null) {
      log(
        "Trending Charts: ${chart.chartItems?.length} items loaded from cache",
        name: "Trending YT Charts",
      );
      return [chart];
    }

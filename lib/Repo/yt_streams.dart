...
class YtStreams {
  ...
  factory YtStreams.fromJson(json) {
    final availableAudioFormats = (json["streamingData"]["adaptiveFormats"]
            as List)
  ...
  List<Audio> get sortByBitrate {
    ...
    audioFormatsCopy.sort(
      (audio1, audio2) => audio1.bitrate.compareTo(audio2.bitrate),
    );
  ...
  List<dynamic> get hmStreamingData {
    ...
    return [true, lowQualityAudio.url, highestQualityAudio.url];
  }
}
...
class Audio {
  ...
  factory Audio.fromJson(json) => Audio(
    audioCodec:
        (json["mimeType"] as String).contains("mp4a") ? Codec.mp4a : Codec.opus,
    itag: json['itag'],

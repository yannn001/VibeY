...
class FileManager {
  static Future<bool> isPlaylistExists(String playlistName) async {
    ...
    final _list = await DBService.getPlaylists4Library();
  ...
  static Future<String?> exportPlaylist(String playlistName) async {
    ...
    final mediaPlaylistDB = await DBService.getPlaylist(playlistName);
    if (mediaPlaylistDB != null) {
      try {
        List<MediaItemDB>? playlistItems = await DBService.getPlaylistItems(
          mediaPlaylistDB,
        );
    ...
    try {
      await readFromJSON(filePath).then((playlistMap) async {
        ...
        if (playlistMap != null && playlistMap.isNotEmpty) {
          bool playlistExists = await isPlaylistExists(
          ...
          for (final mediaItemMap in playlistMap['mediaItems']) {
            ...
            await DBService.addMediaItem(mediaItemDB, playlistName);
            log(
              "Media item imported successfully - ${mediaItemDB.title}",
              name: "FileManager",
            );
          }

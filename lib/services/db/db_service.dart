...
class DBService {
  ...
  factory DBService({String? appSuppPath, String? appDocPath}) {
    if (appSuppPath != null) {
  ...
  static Future<void> checkAndRestoreDB(
    ...
    try {
      ...
      log("Failed to restore DB", error: e, name: "vibeyDBService");
    }
  }
    ...
    if (Isar.instanceNames.isEmpty) {
      ...
      if (!await dbFile.exists()) {
        ...
        await checkAndRestoreDB(dbFile.path, [
          ...
          p.join(appDocDir, 'backup_db.isar'),
          p.join(appSuppDir, 'backup_db.isar'),
        ]);
      }
      ...
      if (!await dbFile.exists() &&
        ...
        log("DB Copied to $appSuppDir", name: "vibeyDBService");
        _db.close();
      }
  ...
  static Future<bool> createBackUp() async {
    try {
      ...
      final File backUpFile = File('$backUpDir/backup_db.isar');
      if (await backUpFile.exists()) {
        await backUpFile.delete();
      }

      await isar.copyToFile('$backUpDir/backup_db.isar');

      log("Backup created successfully ${backUpFile.path}", name: "DBService");
      return true;
    } catch (e) {
      log("Failed to create backup", error: e, name: "DBService");
    }
    return false;
  }
  ...
  static Future<bool> backupExists() async {
    try {
      ...
      final dbFile = File('$backUpDir/backup_db.isar');
      if (dbFile.existsSync()) {
        return true;
      }
    } catch (e) {
      log("No backup exists", error: e, name: "DBService");
    }
    return false;
  }
  ...
  static Future<bool> restoreDB() async {
    try {
      ...
      if (await dbFile.exists()) {
        ...
        log("Successfully restored", name: "DBService");
        DBService();
        return true;
      }
    } catch (e) {
      log("Restoring DB failed", error: e, name: "DBService");
    }
    DBService();
    return false;
  }
  ...
  static Future<void> putSearchHistory(String searchQuery) async {
    ...
    SearchHistoryDB? _searchHistoryDB =
        isarDB.searchHistoryDBs
            .filter()
            .queryEqualTo(searchQuery)
            .findFirstSync();
    if (_searchHistoryDB != null) {
      isarDB.writeTxn(
        () => isarDB.searchHistoryDBs.put(
          _searchHistoryDB..lastSearched = DateTime.now(),
        ),
      );
    } else {
      isarDB.writeTxnSync(
        () => isarDB.searchHistoryDBs.putSync(
          SearchHistoryDB(query: searchQuery, lastSearched: DateTime.now()),
        ),
      );
    }
  }
  ...
  static Future<int?> addPlaylist(MediaPlaylistDB mediaPlaylistDB) async {
    ...
    MediaPlaylistDB? _mediaPlaylist =
        isarDB.mediaPlaylistDBs
            .filter()
            .isarIdEqualTo(mediaPlaylistDB.isarId)
            .findFirstSync();
  ...
  static Future<List<MediaPlaylist>> getPlaylists4Library() async {
    ...
    for (var e in playlists) {
      ...
      mediaPlaylists.add(
        fromPlaylistDB2MediaPlaylist(e, playlistsInfoDB: info),
      );
    }
  ...
  static Future<int> addPlaylistInfo(PlaylistsInfoDB playlistInfoDB) async {
    ...
    return isarDB.writeTxnSync(
      () => isarDB.playlistsInfoDBs.putSync(playlistInfoDB),
    );
  }
  ...
  static Future<void> removePlaylistInfoByName(String playlistName) async {
    Isar isarDB = await db;
    int c = isarDB.writeTxnSync(
      () =>
          isarDB.playlistsInfoDBs
              .filter()
              .playlistNameEqualTo(playlistName)
              .deleteAllSync(),
    );
  ...
  static Future<void> likeMediaItem(
    MediaItemDB mediaItemDB, {
    isLiked = false,
  }) async {
    Isar isarDB = await db;
    addPlaylist(MediaPlaylistDB(playlistName: "Your Likes"));
    MediaItemDB? _mediaItem =
        isarDB.mediaItemDBs
            .filter()
    ...
    if (isLiked && _mediaItem != null) {
      addMediaItem(mediaItemDB, "Your Likes");
  ...
  static Future<bool> isMediaLiked(MediaItemDB mediaItemDB) async {
    ...
    if (_mediaItemDB != null) {
      return (isarDB.mediaPlaylistDBs.getSync(
            MediaPlaylistDB(playlistName: "Your Likes").isarId,
    ...
    if (key.isNotEmpty && value.isNotEmpty) {
      isarDB.writeTxnSync(
        () => isarDB.appSettingsStrDBs.putSync(
          AppSettingsStrDB(settingName: key, settingValue: value),
        ),
      );
    }
  }
  ...
  static Future<void> putSettingBool(String key, bool value) async {
    ...
    if (key.isNotEmpty) {
      isarDB.writeTxnSync(
        () => isarDB.appSettingsBoolDBs.putSync(
          AppSettingsBoolDB(settingName: key, settingValue: value),
        ),
      );
    }
  }
  ...
  static Future<Stream<AppSettingsBoolDB?>?> getWatcher4SettingBool(
    ...
    if (id != null) {
      ...
      isarDB.writeTxnSync(
        () => isarDB.appSettingsBoolDBs.putSync(
          AppSettingsBoolDB(settingName: key, settingValue: false),
        ),
      );
  ...
  static Future<void> putRecentlyPlayed(MediaItemDB mediaItemDB) async {
    ...
    if (_mediaItemDB != null) {
      RecentlyPlayedDB? _recentlyPlayed =
          isarDB.recentlyPlayedDBs
              .filter()
              .mediaItem((q) => q.idEqualTo(_mediaItemDB.id!))
              .findFirstSync();
      if (_recentlyPlayed != null) {
        isarDB.writeTxnSync(
          () => isarDB.recentlyPlayedDBs.putSync(
            _recentlyPlayed..lastPlayed = DateTime.now(),
          ),
        );
      } else {
        isarDB.writeTxnSync(
  ...
  static Future<void> refreshRecentlyPlayed() async {
    ...
    int days = int.parse(
      (await getSettingStr(
        GlobalStrConsts.historyClearTime,
        defaultValue: "7",
      ))!,
    );
  ...
  static Future<void> putChart(ChartModel chartModel) async {
    ...
    isarDB.writeTxnSync(
      () =>
          _id = isarDB.chartsCacheDBs.putSync(
            chartModelToChartCacheDB(chartModel),
          ),
    );
  ...
  static Future<void> removeDownloadDB(MediaItemModel mediaItem) async {
    ...
    try {
      ...
      log(
        "Failed to delete file: ${downloadDB!.fileName}",
        error: e,
        name: "DB",
      );
    }
  ...
  static Future<void> putOnlPlaylistModel(PlaylistModel playlistModel) async {
    Isar isarDB = await db;
...
PlaylistModel formatSavedPlaylistOnl(SavedCollectionsDB savedCollectionsDB) {
  Map extra = jsonDecode(savedCollectionsDB.extra ?? "{}");

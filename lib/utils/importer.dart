...
import 'package:vibey/Repo/Spotify/spotify_api.dart';
    ...
    if (playlistID != null) {
      ...
      try {
        ...
        await for (var video in tempStrm) {
          {
            var itemMap = await YouTubeServices().formatVideo(
              video: video,
              quality: "High",
              getUrl: false,
            );
            var item = fromYtVidSongMap2MediaItem(itemMap!);
            await DBService.addMediaItem(
  ...
  static Stream<ImporterState> ytmPlaylistImporter(String url) async* {
    ...
    if (playlistID != null) {
      try {
        ...
        log(
          "Playlist Name: ${playlistMeta['name']}",
  ...
  static Future<MediaItemModel?> ytMediaImporter(String url) async {
    ...
    if (videoId != null) {
      try {
        ...
        final itemMap = await YouTubeServices().formatVideo(
          video: video,
          quality: "High",
          getUrl: false,
        );
  ...
  static Stream<ImporterState> sfyPlaylistImporter({
    ...
    if (playlistID != null) {
      ...
      try {
        ...
        final data = await SpotifyApi().getAllTracksOfPlaylist(
          accessToken,
          playlistID,
        );
